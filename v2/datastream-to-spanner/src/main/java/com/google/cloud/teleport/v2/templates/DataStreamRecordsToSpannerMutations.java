    ...
    PCollectionTuple spannerMutations = datastreamJsonRecords.apply("Convert Datastream record to Spanner mutation",
        ParDo.of(new DataStreamRecordsToSpannerMutationsDoFn(transformationContext, ddlView, schema, sourceType))
            ...
            .withOutputTags(
                ...
                TupleTagList.of(PERMANENT_ERROR_TAG).and(RETRYABLE_ERROR_TAG)));
    return Result.create(spannerMutations.get(SUCCESSFUL_EVENT_TAG).setCoder(SerializableCoder.of(Mutation.class)),
        spannerMutations.get(PERMANENT_ERROR_TAG).setCoder(FailsafeElementCoder.of(StringUtf8Coder.of(), StringUtf8Coder.of())),
        spannerMutations.get(RETRYABLE_ERROR_TAG).setCoder(FailsafeElementCoder.of(StringUtf8Coder.of(), StringUtf8Coder.of())));
  }
